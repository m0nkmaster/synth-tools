/**
 * SynthPrototypeE - Teenage Engineering Inspired (Full Featured)
 * Complete synthesis UI with all SoundConfig features
 */
...
// ═══════════════════════════════════════════════════════════════════════════════
// ENVELOPE DISPLAY (Full size, matches waveform)
// ═══════════════════════════════════════════════════════════════════════════════
...
// Mini module block for layer features
    ...
    <div onClick={onSelect} style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }} onClick={e => e.stopPropagation()}>
          <input type="range" min={0} max={1} step={0.01} value={layer.gain}
          ...
          {canRemove && (
            <button onClick={onRemove} style={{
              width: 16, height: 16, border: 'none', background: `${TE.pink}20`,
              borderRadius: 2, fontSize: 10, color: TE.pink, cursor: 'pointer', fontWeight: 700,
            }}>×</button>
          )}
        </div>
      </div>
      ...
      {selected && (
        <div style={{ padding: 10 }}>
          {/* OSCILLATOR - 2x2 grid of modules */}
...
function Effect({ name, on, onToggle, color, children }: EffectProps) {
  return (
...
export default function SynthPrototypeE() {
  const {
    ...
    updateEnvelope, updateFilter, updateLFO, updateEffects, updateDuration,
    play, exportWav,
  } = useSynthState();
  ...
  // Access config with type to update dynamics
  ...
  return (
    <div style={{ minHeight: '100vh', background: TE.bg, fontFamily: '-apple-system, BlinkMacSystemFont, "Helvetica Neue", sans-serif' }}>
      {/* HEADER */}
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', background: TE.panel, borderBottom: `1px solid ${TE.border}` }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={play} disabled={playing} style={{ padding: '6px 14px', background: playing ? TE.surface : TE.orange, border: 'none', borderRadius: 3, color: playing ? TE.grey : '#fff', fontSize: 9, fontWeight: 700, cursor: playing ? 'not-allowed' : 'pointer' }}>
            {playing ? '■ PLAYING' : '▶ PLAY'}
          </button>
          <button onClick={exportWav} disabled={exporting} style={{ padding: '6px 14px', background: TE.panel, border: `1px solid ${TE.green}`, borderRadius: 3, color: TE.green, fontSize: 9, fontWeight: 700, cursor: exporting ? 'not-allowed' : 'pointer', opacity: exporting ? 0.5 : 1 }}>↓ EXPORT</button>
        </div>
      </header>
